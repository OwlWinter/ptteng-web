 ...
 * @Description 需要登录才能看到 /u/ 下的页面，这里主要负责这些页面的鉴权
 * @Author owlwinter
...
public class UserInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        boolean tokenStatus = UserToken.verifyToke(token);

        LOGGER.info("==> [preHandle] token:{}", token);

        if (tokenStatus == false){
            String call_back = request.getRequestURI();
            String redirectUrl = "/login?call_back=" + call_back;
            response.sendRedirect(redirectUrl);

            LOGGER.info("<== [preHandle] token 校验失败 redirect:{}", redirectUrl);
        } else {
            LOGGER.info("<== [preHandle] token 校验通过 playload:{}", UserToken.parser(token));
